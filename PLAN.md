...
**Second half — SHIPPED `7177c931` 2026-05-26 (with caveats).** `canary_transcribe_streamed` lands as a parakeet-pattern long-audio path: full-mel + 8 s/2 s overlapping chunked encode + concat → single AED decode. `canary_transcribe_ex`'s post-encode body (cross-KV + prompt + greedy decode + DTW timestamps) was extracted into a static `canary_finish_from_encoder` helper that both entry points share. The backend wrapper at `crispasr_backend_canary.cpp` routes through streamed for n_samples > 30 s with `CANARY_STREAM_THRESHOLD_S` override; default behaviour on JFK is unchanged (single-pass path).

**Known limitation.** On synthetic concatenated short clips (`jfk × 8 = 88 s`) the streamed path emits `<eos>` at the chunk boundary after producing one full JFK transcript. Forcing streamed on plain JFK with `CANARY_STREAM_THRESHOLD_S=0` also truncates to the first sentence ("...for you." instead of "...for you, ask what you can do for your country."). This is the AED-trained-on-single-utterance issue: canary's decoder treats the splice point as an utterance end. NeMo's `FrameBatchMultiTaskAED` handles this with per-chunk prompt re-injection; we don't yet. For natural long-audio (one continuous utterance, no internal silences masquerading as utterance ends) the streamed path should produce a coherent transcript because there's no mid-stream `<eos>` signal — but this needs a real long-form en/de/fr/es clip to validate. Filed as a follow-up: implement per-chunk prompt re-injection in `canary_transcribe_streamed`, then promote `CANARY_STREAM_THRESHOLD_S` default to `0` (always streamed) per the parakeet pattern.
