...
static bool ws_send_text(socket_t fd, const std::string& text) {
    ...
    if (n < 126) {
        ...
        head[1] = 127;
        for (int i=0; i<8; i++) head[2+i] = (uint8_t)(n >> (56 - 8*i));
        hlen = 10;
    }
    if (!send_all(fd, head, hlen)) return false;
...
static int ws_read_frame(socket_t fd, std::vector<uint8_t>& payload, uint8_t* out_opcode) {
    ...
    if (len == 126) {
        uint8_t e[2]; if (!recv_exact(fd, e, 2)) return -1;
        len = ((uint64_t)e[0] << 8) | e[1];
    } else if (len == 127) {
        uint8_t e[8]; if (!recv_exact(fd, e, 8)) return -1;
    ...
    if (len > 1024*1024*10) return -1; // 10MB limit
    payload.resize((size_t)len);
    if (len > 0) {
        if (!recv_exact(fd, payload.data(), (size_t)len)) return -1;
        if (masked) {
...
struct rt_session {
    ...
    void process_audio() {
        ...
        backend->transcribe_streaming(audio_buffer.data(), audio_buffer.size(), 0, rp,
            [&](const std::string& partial, bool is_final) {
                current_buffer_text = partial;
            });
...
static void rt_handle_connection(rt_session* sess) {
    ...
    if (req_len <= 0) { CLOSE_SOCKET(sess->client_fd); delete sess; return; }
    req_buf[req_len] = '\0';
...
int realtime_server_start(CrispasrBackend* backend, std::mutex& model_mutex, const whisper_params& base_params, int port) {
    if (g_rt_running.load()) return 0;
#ifdef _WIN32
    WSADATA wsa;
    ...
    if (g_rt_listen_fd == INVALID_SOCKET) return -1;
